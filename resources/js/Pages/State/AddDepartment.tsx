...
const AddDepartment = ({ handleCancel }: AddDepartmentProps) => {
    ...
    const handleStateChange = (value: string) => {
        if (/^(?!0)\d{0,3}$/.test(value)) setData('state', value);
    };
        ...
        <div className="mt-4 p-4 border border-indigo-300 shadow-xl bg-[repeating-linear-gradient(45deg,#f5f3ff,#f5f3ff_10px,#e0e7ff_10px,#e0e7ff_20px)]">
            <div className="bg-white p-2 border border-indigo-200 shadow-sm">
                <table className="min-w-full">
                    ...
                    <tbody>
                        <tr className="bg-white">
                            ...
                            <td className="px-1 py-1">
                                <select value={data.territory} onChange={e => setData('territory', e.target.value)} className={`w-32 bg-white border p-1 text-sm font-mono uppercase outline-none ${getBorderClass('territory')}`}>
                            ...
                            <td className="px-1 py-1 text-right">
                                <div className="flex justify-end">
                                    <input placeholder="100" value={data.state} onChange={e => handleStateChange(e.target.value)} className={`w-20 bg-white border p-1 text-sm font-mono text-right outline-none ${getBorderClass('state')}`} />
                                </div>
                            </td>
                        </tr>
                    </tbody>
                ...
                <div className="mt-4 flex justify-end gap-2 border-t border-indigo-100 pt-2">
                    <button onClick={handleCancel} className="bg-white text-indigo-600 border border-indigo-600 px-3 py-1 text-xs font-bold whitespace-nowrap hover:bg-indigo-50 transition-colors lowercase cursor-pointer">ОТМЕНИТЬ</button>
                    <button onClick={handleAdd} disabled={processing} className="bg-indigo-600 text-white px-3 py-1 text-xs font-bold whitespace-nowrap hover:bg-indigo-700 transition-colors disabled:opacity-50 lowercase cursor-pointer">ДОБАВИТЬ</button>
                </div>
            </div>
        </div>
    );

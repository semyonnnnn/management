...
function getGradientColor(percent: number) {
    if (percent < 45) return "from-amber-400 via-amber-500 to-orange-500";
    if (percent <= 60) return "from-emerald-400 via-emerald-500 to-teal-500";
    return "from-rose-500 via-red-500 to-red-600";
}

function getStatusText(percent: number) {
    if (percent < 45) return "МАЛАЯ ЗАГРУЗКА";
    if (percent <= 60) return "ОПТИМАЛЬНО";
    return "ПЕРЕГРУЗКА";
}

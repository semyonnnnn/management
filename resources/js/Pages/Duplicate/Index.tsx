...
export default function Index({ departments, forms, filters }: ExtendedPageProps) {
    ...
    // Central Filter Pipeline Worker: Always submits BOTH parameters together
    ...
    // Keep state values locked with server props sync shifts
    useEffect(() => {
        setSelectedTerritory(filters.territory || 'all');
    }, [filters.territory]);

    // Handle automated text typing stream logic cleanly via debounce
    useEffect(() => {
            ...
            <div className="space-y-6" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                ...
                <div className="bg-white border border-indigo-200/50 p-6 flex flex-col xl:flex-row gap-4 justify-between items-stretch xl:items-center shadow-sm">
                    <div className="flex items-center gap-4">
                        <div className="w-fit h-9 px-3 bg-indigo-600 flex items-center justify-center text-white text-sm font-bold font-mono shrink-0">
                            (╯°□°）╯︵ ┻━┻
                        </div>
                        <div>
